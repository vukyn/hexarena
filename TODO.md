...
⚠️ **Ticking it means MOVING it.** The entry goes, whole and unedited, to
`docs/decisions.md` under its code, and its row in § *The codes* changes status
in that same commit. § *Not done* keeps open items only — a `- [x]` under that
heading is the defect, not the record.
...
is the file working rather than failing.

⚠️ **`## Not done` is only what is not done — and that sentence is a
measurement, not a virtue.** What stood here until 2026-09-13 asserted that the
2026-09-05 sweep had fixed the headings and that they now say what they hold. It
was true for **eight days**. By 2026-09-13 § *Not done* held **thirty finished
entries against twelve open ones** — 2,663 lines of completed work filed as
unfinished — because the way to close an item was to change `[ ]` to `[x]` where
it sat. Finished work and its reasoning live in `docs/decisions.md`, under the
item's own code, which is where to look for why a shipped thing is the way it is.

**The claim is now one command, and the command has to print nothing:**
...
evidence that it works, only a command somebody believes in. It stays a command
rather than a paragraph for the reason the paragraph it replaced demonstrates —
prose about a file cannot notice the file changing under it. `DOC-001` is this
sweep's own entry and records what it cost to find that out.
...
| `DOC` | **doc**umentation | this file, `docs/`, `CLAUDE.md`, `README.md` and `memory/` — what the repository writes down about itself |
...
**status** is `open` (not done — the entry is in § *Not done*), `done` (finished
— the entry, with its measurements, is in `docs/decisions.md` under the same
code), `shipped` (finished, and § *Done* carries a paragraph on the capability)
or `refused` (§ *Decided against*).

⚠️ **`done` used to mean *"entry kept in § Not done with its measurements"*, and
that rule is RETIRED.** It is the rule that let thirty finished entries pile up
under a heading that says they are not done. A `done` item is **moved**, not
ticked in place. **`shipped` survives as its own word** because it says the one
thing `done` cannot — the record is the capability prose in § *Done*, not a
`docs/decisions.md` entry — and collapsing the two would have made this column
stop telling a reader which file to open. The two are not a quality ranking:
`shipped` is a capability the game has, `done` is a question that was answered.
...
| `CLI-003` | done | `--replay` sized its read buffer from the file it was pointed at (`os.ReadFile`) — self-inflicted, since the path is the reader's own argv, but "I meant to type the log" is how it happens — DONE. `io.LimitReader` at 64MiB, DERIVED: a measured log is 67,447 bytes for 255 events, and the turn cap puts a pessimistic ceiling near 106MB |
| `CLI-002` | done | `--replay` hands a saved log's free text to a terminal, which reads an ESC as a command, and printed its `unverified:` notice LAST — so a crafted file both wrote the reader's clipboard (OSC 52) and scrolled the only "nothing checked this" line off the top, leaving a FORGED `verified: … all 255 events exactly` as the last verdict-shaped line — DONE. The notice moved ahead of the body, and `tui.Line` cleans all NINE strings on an event rather than the three the finding named: `actor` was called safe and is printed raw whenever the tags map has no entry for it |
...
| `FRG-005` | done | `replaceFile` threw away `temp.Close()` on the write-error path (gosec G104) — the one path where the close carries something the write does not, because a buffered write can report success and fail on the flush, so a failing close is how a TRUNCATED temp file announces itself — DONE, both errors joined, the write named first. ⚠️ The branch needs BOTH a failing write and a failing close and is not reachable under test without injecting a writer into the one function every write in the package funnels through; not refactored for a LOW finding, and said so rather than shipping a test that proves nothing |
| `NET-001` | open | PvP over a LAN — 3v3 or 5v5, one server and n clients |
| `NET-004` | done | The default room is OPEN to the local network and the banner did not say so — the listener binds every interface, `-browse` announces it over mDNS, and an empty password admits any hello past the version gate; the `password none` line read *"anybody with the code can join"*, which describes a different program — DONE, by making the exposure VISIBLE. ⚠️ A generated default password was the alternative and is REFUSED: the plaintext-`ws://` transport is a stated decision, so a password crossing it is an access token and not a secret, and generating one would change the model of every existing invocation silently |
| `NET-005` | done | The host created its log directory 0755 around 0600 files (gosec G301), listing every match's room code, battle number and seed to anybody with an account on the machine — DONE, 0700, asserted on the mode the filesystem ends up with rather than on the constant, because a umask makes those two different questions |
| `NET-003` | done | A peer's `wire.Hello.Name` reached the host's terminal as COMMANDS — 32 runes is ample for a complete OSC 52, `playerName` bounded and stripped nothing, and a browsable room needs no password, so anybody on the segment could write the host's clipboard — DONE. Cleaned and bounded at the protocol (`wire.CleanName`, run by `Hello.UnmarshalJSON` and again by `Room.Join`). ⚠️ Fixing it in `Room.Join` alone would NOT have worked: the printed name comes off the hello in `internal/socket`, not off the seat |
...
| `DOC-002` | done | Two hygiene items on a PUBLIC remote: gitleaks reported the same false positive on every scan (a documentation example inside the note ABOUT credential literals), and `memory/gitguardian-dashboard-not-repo-file.md` named four PRIVATE platform repos as the real blind spot for live keys in test fixtures — a targeting hint in a world-readable file — DONE. `.gitleaksignore` + `.gitleaks.toml` as a PAIR (the ignore file's own fingerprints scan as secrets without the allowlist), and the sentence restated by PROPERTY rather than by repo name, lesson and wikilinks intact |
