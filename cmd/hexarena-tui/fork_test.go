...
// of them has to move the listing behind it and re-push — the same arrangement
// the level already has, and the same one that made the preview a screen that
// could live in internal/screen at all. A form key wired into the browser and
// not into `updatePreview` would leave a reader looking at a picture, pressing
// the key the picture itself offers, and watching nothing happen.

// TestTheFormKeyWalksTheArmsFromEitherDescriber presses `s` on the two screens a
// reader can be on when they see the form row, and reads the drawn screen back.
func TestTheFormKeyWalksTheArmsFromEitherDescriber(t *testing.T) {
	base, _, _ := start(t, i18n.En)
	forked := theForkedBrowser(t, base)
	character := forked.cast.Rows()[forked.cast.Cursor]
	arms, err := character.FurthestAt(forked.cast.Level)
	if err != nil || len(arms) < 2 {
		t.Fatalf("%s does not fork at level %d", character.ID, forked.cast.Level)
	}
	...
	for _, raise := range []struct {
		...
		// And the listing behind moved with it, which is what makes going back
		// land on the arm the reader chose rather than on the one it opened with.
		if got := walked.cast.Subject().Stage; got != arms[1].Name {
			t.Errorf("s on %s left the listing behind on %q, want %s",
...
func TestTheFormKeyDoesNothingOnALineThatDoesNotFork(t *testing.T) {
	base, _, _ := start(t, i18n.En)
	browser := base.enter(screenCast)
	before := browser.cast.Subject()
	arms, err := browser.cast.Rows()[browser.cast.Cursor].FurthestAt(browser.cast.Level)
	if err != nil || len(arms) != 1 {
		t.Fatalf("the row this listing opens on forks, so this measures the wrong case")
	}
	if drawn := drawnBody(browser); strings.Contains(drawn, "< "+arms[0].Name+" >") {
		t.Errorf("a line that does not fork draws a form chooser:\n%s", drawn)
	}
	if after := key(t, browser, "s").cast.Subject(); after != before {
