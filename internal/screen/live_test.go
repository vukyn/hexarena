...
func TestALiveRedrawDrawsTheBattleAsItStandsNow(t *testing.T) {
	...
	if opening != tui.RosterFor(c.Lang, fight, live.Tags, c.UsableWidth(), c.Style.ElementInk()) {
	...
	now := tui.RosterFor(c.Lang, fight, live.Tags, c.UsableWidth(), c.Style.ElementInk())
	if now == opening {
