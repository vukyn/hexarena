...
// BuildOf is the build string a binary announces, from the three sources in the
...
//  3. **Unstamped**, for a `go run` — which records no revision at all, so this
//     is the ordinary case during development rather than an error path.
//
...
// second binary needs the same three-step fallback, and a second spelling of a
// fallback is how two peers come to disagree about what they are.
